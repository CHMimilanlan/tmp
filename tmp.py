...
class LtxvTrainer:
    ...
    def _collect_trainable_params(self) -> None:
        ...
        for name, param in (
            ...
            if (
                ...
                av2av_numel += param.numel()

            elif (
                "spatial_track_encoder."
                "modules_to_save.default" in name
            ):
                spatial_numel += param.numel()

            else:
                unexpected_numel += param.numel()
                logger.warning(
        ...
        if not self._trainable_params:
            raise RuntimeError(
                "No trainable parameters were collected."
            )

        if av2av_numel == 0:
            raise RuntimeError(
                "AV2AV LoRA has zero trainable parameters."
            )
        ...
        logger.info(
            ...
            f"  AV2AV LoRA:          {av2av_numel:,}\n"
            f"  SpatialTrackEncoder: {spatial_numel:,}\n"
            f"  Unexpected:          {unexpected_numel:,}\n"
            f"  Total:               "
    ...
    def _setup_lora(self) -> None:
        """
        Configure training as:

        1. Load motion-track IC-LoRA
        2. Locate its LoRA layers
        3. Merge it into the LTX transformer
        4. Freeze the merged transformer
        5. Add a new trainable AV2AV LoRA
        6. Train SpatialTrackEncoder through modules_to_save
        """
        logger.info(
        ...
        # ---------------------------------------------------------
        # 1. Load and freeze motion-track LoRA by merging it
        # ---------------------------------------------------------
        self._load_and_merge_motion_track_lora()

        # 此时 self._transformer 已经是：
        # 原始 LTX + motion-track LoRA 增量
        # 并且全部 frozen。
        assert not any(
            param.requires_grad
            for param in self._transformer.parameters()
        ...
        # ---------------------------------------------------------
        # 2. Create the new trainable AV2AV LoRA
        # ---------------------------------------------------------
        av2av_lora_config = LoraConfig(
            r=self._config.lora.rank,
            lora_alpha=self._config.lora.alpha,
            target_modules=(
                self._config.lora.target_modules
            ),
            lora_dropout=self._config.lora.dropout,
            init_lora_weights=True,
            ...
            # 只属于最终的 AV2AV/default adapter。
            #
            # PEFT 会：
            # 1. 创建 SpatialTrackEncoder 的 trainable copy
            # 2. 冻结 original_module
            # 3. 保存 trainable copy 到 AV2AV checkpoint
            modules_to_save=[
                "spatial_track_encoder",
            ],
        )
        ...
        logger.info(
            "Creating trainable AV2AV LoRA:\n"
            f"  adapter={AV2AV_ADAPTER}\n"
            f"  rank={self._config.lora.rank}\n"
            f"  alpha={self._config.lora.alpha}\n"
            f"  targets={self._config.lora.target_modules}"
        )

        # 不传 adapter_name，使其继续叫 default。
        # 这样当前 LTX 的保存、恢复和 _init_lora_weights
        # 基本都可以保持兼容。
        ...
        #     inference_mode=False,
        # )
        self._transformer.set_adapter(AV2AV_ADAPTER)

        # ---------------------------------------------------------
        # 3. Audit LoRA and SpatialTrackEncoder
        # ---------------------------------------------------------
        audit_lora_model(self._transformer)

        self._audit_av2av_and_spatial_track_encoder()
    ...
    def _load_and_merge_motion_track_lora(
        self,
    ) -> None:
        """
        Load the pretrained motion-track IC-LoRA, locate its
        injected layers, freeze it, and merge it into the
        underlying LTX transformer.

        After this function:
        - motion-track behavior is baked into base weights
        - no motion_track lora_A/lora_B parameters remain
        - the entire transformer is frozen
        """
        cfg = self._config.lora
        checkpoint_path = cfg.motion_track_checkpoint
        ...
        # Important:
        # merging a LoRA into already-quantized weights is not
        # recommended. See the quantization note below.
        if self._config.acceleration.quantization is not None:
            raise RuntimeError(
                "Motion-track LoRA is currently being merged "
                "inside _setup_lora(), but the LTX transformer "
                "has already been quantized in _load_models(). "
                "Either set acceleration.quantization=null, or "
                "move motion-track merging before quantize_model()."
            )
        ...
        motion_alpha = cfg.motion_track_alpha

        if motion_alpha is None:
            motion_alpha = motion_rank

            logger.warning(
                "motion_track_alpha was not specified. "
                f"Using alpha=rank={motion_rank}, which gives "
                "LoRA scaling=1. Verify this against the "
                "original motion-track training configuration."
            )

        motion_config = LoraConfig(
            r=motion_rank,
            lora_alpha=motion_alpha,

            # 使用 checkpoint 中精确定位的完整模块名
            ...
            # motion-track adapter 不负责 SpatialTrackEncoder
            modules_to_save=None,
        )

        logger.info(
            "Creating temporary motion-track adapter:\n"
            f"  adapter={MOTION_TRACK_ADAPTER}\n"
            f"  rank={motion_rank}\n"
            f"  alpha={motion_alpha}\n"
            f"  scaling={motion_alpha / motion_rank:.6f}"
        )

        motion_peft_model = get_peft_model(
            self._transformer,
            motion_config,
            adapter_name=MOTION_TRACK_ADAPTER,
        )

        # 将 checkpoint 权重装入 motion_track adapter
        load_result = set_peft_model_state_dict(
            motion_peft_model.get_base_model(),
            motion_state_dict,
            adapter_name=MOTION_TRACK_ADAPTER,
        )
        ...
        if unexpected_keys:
            raise RuntimeError(
                "Unexpected motion-track LoRA checkpoint keys:\n"
                + "\n".join(unexpected_keys[:100])
            )

        # missing_keys 中通常会包含大量未加载的基础模型权重。
        # 这里只检查 motion-track adapter 自己的 LoRA 参数。
        ...
        if adapter_missing_keys:
            raise RuntimeError(
                "Some motion-track LoRA tensors were not loaded:\n"
                + "\n".join(adapter_missing_keys[:100])
            )

        motion_layers = _locate_named_lora_adapter(
            motion_peft_model,
        ...
        if not motion_layers:
            raise RuntimeError(
                "Motion-track checkpoint was loaded, but no "
                "motion_track LoRA layers were found in the model."
            )

        logger.info(
            f"Located {len(motion_layers)} motion-track "
            "LoRA layers in the PEFT model:\n"
            + "\n".join(
                f"  [{index:04d}] {name}"
                for index, name in enumerate(motion_layers)
            )
        )

        # 冻结临时 motion-track adapter
        motion_peft_model.set_requires_grad(
            MOTION_TRACK_ADAPTER,
            requires_grad=False,
        )

        trainable_motion_params = [
            name
            for name, param
            in motion_peft_model.named_parameters()
            if (
                MOTION_TRACK_ADAPTER in name
                and "lora_" in name
                and param.requires_grad
            )
        ]

        if trainable_motion_params:
            raise RuntimeError(
                "Motion-track LoRA should be frozen, but the "
                "following parameters remain trainable:\n"
                + "\n".join(trainable_motion_params[:100])
            )

        # 将 motion-track LoRA 永久合并进基础 LTX 权重。
        #
        # merge_and_unload 不是原地返回 PeftModel，
        # 必须接收其返回值。
        merged_transformer = (
            motion_peft_model.merge_and_unload(
                safe_merge=True,
                adapter_names=[
                    MOTION_TRACK_ADAPTER,
                ],
        ...
        # 冻结“原始 LTX + motion-track 增量”
        merged_transformer.requires_grad_(False)

        remaining_motion_lora = [
            name
            for name, _ in merged_transformer.named_parameters()
            if (
                MOTION_TRACK_ADAPTER in name
                or "lora_A.motion_track" in name
                or "lora_B.motion_track" in name
            )
        ]

        if remaining_motion_lora:
            raise RuntimeError(
                "Motion-track LoRA parameters remain after "
                "merge_and_unload:\n"
                + "\n".join(remaining_motion_lora[:100])
            )

        remaining_tuner_layers = [
            name
            for name, module
            in merged_transformer.named_modules()
            if isinstance(module, BaseTunerLayer)
        ]

        if remaining_tuner_layers:
            raise RuntimeError(
                "Temporary PEFT tuner layers remain after merging:\n"
                + "\n".join(remaining_tuner_layers[:100])
            )
        ...
        if not hasattr(
            ...
            raise RuntimeError(
                "SpatialTrackEncoder disappeared while merging "
                "the motion-track LoRA."
            )

        self._transformer = merged_transformer

        logger.info(
            "Motion-track IC-LoRA was merged successfully. "
            "Its effect is now frozen in the base transformer."
        )
