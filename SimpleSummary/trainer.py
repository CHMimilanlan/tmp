...
class LtxvTrainer:
    def __init__(self, trainer_config: LtxTrainerConfig) -> None:
        ...
        # 内部会先调用 _setup_lora()，建立完整 AV2AV LoRA
        self._collect_trainable_params()

        self._loaded_checkpoint_path: Path | None = None
        if self._config.model.load_checkpoint is not None:
            # 恢复你自己训练的 AV2AV LoRA + SpatialTrackEncoder checkpoint
            if (
                self._config.lora is not None
                and self._config.lora.init_checkpoint is not None
            ):
                logger.warning(
                    "Both model.load_checkpoint and lora.init_checkpoint are configured. "
                    "The resume checkpoint takes precedence; init_checkpoint is ignored."
                )

            self._load_checkpoint()
        else:
            # 首次训练：使用 Motion Track LoRA 初始化当前 AV2AV LoRA
            self._load_lora_initialization_checkpoint()
    ...
    def _load_models(self) -> None:
        ...
        if self._config.acceleration.quantization is not None:
            if self._config.model.training_mode == "full":
                raise ValueError(
                    "Quantization is not supported in full training mode."
                )

            logger.info(
                f'Quantizing model with '
                f'"{self._config.acceleration.quantization}". '
    ...
    def _load_lora_checkpoint(
        ...
        """
        Completely restore an AV2AV LoRA checkpoint.

        Unlike _load_lora_initialization_checkpoint(), this checkpoint is
        expected to contain:
        - every current AV2AV LoRA tensor
        - the full SpatialTrackEncoder saved through modules_to_save
        """
        checkpoint_file_state = load_file(
            str(checkpoint_path),
            device="cpu",
        )

        checkpoint_state: dict[str, Tensor] = {
            self._normalize_peft_checkpoint_key(key): tensor
            for key, tensor in checkpoint_file_state.items()
        }

        current_state, canonical_to_native = (
            self._get_current_peft_state_and_key_map()
        )

        expected_keys = set(canonical_to_native)
        checkpoint_keys = set(checkpoint_state)

        missing_keys = sorted(expected_keys - checkpoint_keys)
        unexpected_keys = sorted(checkpoint_keys - expected_keys)

        mismatched_shapes: list[
            tuple[str, tuple[int, ...], tuple[int, ...]]
        ] = []

        for canonical_key in expected_keys & checkpoint_keys:
            native_key = canonical_to_native[canonical_key]

            source_shape = tuple(
                checkpoint_state[canonical_key].shape
            )
            destination_shape = tuple(
                current_state[native_key].shape
            )
        ...
        if missing_keys or unexpected_keys or mismatched_shapes:
            message_parts = [
                "The AV2AV LoRA checkpoint is incompatible with the "
                "current PEFT model."
            ]
            ...
            if unexpected_keys:
                message_parts.append(
                    "Unexpected keys:\n"
                    + "\n".join(
                        f"  - {key}" for key in unexpected_keys[:20]
                    )
                )
            ...
            if mismatched_shapes:
                message_parts.append(
                    ...
                    + "\n".join(
                        (
                            f"  - {key}: checkpoint={src_shape}, "
                            f"model={dst_shape}"
                        )
                        for key, src_shape, dst_shape
                        in mismatched_shapes[:20]
                    )
                )

            raise RuntimeError("\n\n".join(message_parts))

        # Restore the PEFT-native key names before passing to PEFT.
        peft_native_state: dict[str, Tensor] = {}

        for canonical_key, native_key in canonical_to_native.items():
            destination_tensor = current_state[native_key]
            source_tensor = checkpoint_state[canonical_key]

            peft_native_state[native_key] = source_tensor.to(
        ...
        logger.info(
            "✅ Complete AV2AV LoRA + SpatialTrackEncoder checkpoint "
            f"loaded successfully: {checkpoint_path}"
        )
    ...
    def _prepare_models_for_training(self) -> None:
        ...
        if (
            ...
            fsdp_plugin = self._accelerator.state.fsdp_plugin

            if fsdp_plugin is None:
                raise RuntimeError(
                    "FSDP distributed type is active, but "
                    "accelerator.state.fsdp_plugin is None."
                )
            ...
            logger.info(
                "Installed PEFT-aware FSDP auto-wrap policy."
            )
        ...
        # 非常重要：
        # FSDP prepare 之后重新取得当前真正由 FSDP 管理的参数。
        # self._trainable_params = [
        #     parameter
        ...
        logger.info(
            "Trainable parameters after FSDP wrapping: "
    ...
    def _init_dataloader(self) -> None:
        ...
        if self._dataset is None:
            data_sources = (
                self._config.training_strategy.get_data_sources()
            )

            # -----------------------------------------------------
            # Track-PRoPE configuration validation
            # -----------------------------------------------------
            if self.use_track_prope or self.use_ic_lora_summary:
                if "track" not in data_sources.values():
                    raise ValueError(
                        "Track-PRoPE is enabled, but no track data source "
                        "was configured.\n\n"
                        "Add this under training_strategy.audio:\n"
                        "  track: track"
                    )

            logger.info(
                "Precomputed data sources:\n"
                + "\n".join(
                    (
                        f"  {directory_name} "
                        f"-> batch[{output_key!r}]"
                    )
                    for directory_name, output_key
                    in data_sources.items()
                )
            )
            ...
            logger.info(
                ...
                f"{len(self._dataset):,} samples."
            )

        num_workers = (
            self._config.data.num_dataloader_workers
        )
    ...
    def _save_checkpoint(self) -> Path | None:
        ...
        #
        # 重要：
        # 不要调用 block.audio_track_prope.state_dict()。
        ...
        for raw_key, value in full_state_dict.items():
            ...
            # FSDP/PEFT 下可能出现：
            #
            # base_model.model.transformer_blocks.0.audio_track_prope.xxx
            # _fsdp_wrapped_module.base_model.model.transformer_blocks...
            #
            # 从 transformer_blocks 开始截断，得到稳定的保存键。
            transformer_marker = "transformer_blocks."
            track_marker = ".audio_track_prope."
            ...
            if track_marker not in normalized_key:
                continue

            if normalized_key in track_prope_state_dict:
                raise RuntimeError(
                    "Duplicate Track-PRoPE checkpoint key detected: "
                    f"{normalized_key}\n"
                    f"Original full-state key: {raw_key}"
                )

            track_prope_state_dict[normalized_key] = (
                value.detach()
                .to(
                    device="cpu",
                    dtype=save_dtype,
        ...
        #
        # 重要：
        # 与 Track-PRoPE 相同，不要调用子模块的 state_dict()。
        # full_state_dict 已经由所有 rank 共同完成了 FSDP gather。
        # 这里只进行普通 Python 字典筛选，不会触发新的 collective。
        #
        # Track-Summary 相关的额外模块包括：
        ...
        for raw_key, value in full_state_dict.items():
            ...
            if normalized_key in track_summary_state_dict:
                raise RuntimeError(
                    "Duplicate Track-Summary checkpoint key detected: "
                    f"{normalized_key}\n"
                    f"Original full-state key: {raw_key}"
                )

            track_summary_state_dict[normalized_key] = (
                value.detach()
                .to(
                    device="cpu",
                    dtype=save_dtype,
        ...
        if track_prope_state_dict:
            logger.info(
                "Track-PRoPE saved to "
                f"{track_prope_path}"
            )

        if track_summary_state_dict:
            logger.info(
    ...
    def _get_current_peft_state_and_key_map(
        self,
    ) -> tuple[dict[str, Tensor], dict[str, str]]:
        """
        Returns:
            current_state:
                PEFT-native state dict, including the correct base_model.model prefix.

            canonical_to_native:
                canonical checkpoint key -> actual PEFT state-dict key.
        """
        ...
        for native_key in current_state:
            canonical_key = self._normalize_peft_checkpoint_key(native_key)

            if canonical_key in canonical_to_native:
                raise RuntimeError(
                    "Duplicate normalized PEFT key detected:\n"
                    f"  key={canonical_key}\n"
                    f"  first={canonical_to_native[canonical_key]}\n"
                    f"  second={native_key}"
                )

            canonical_to_native[canonical_key] = native_key

        return current_state, canonical_to_native


    def _load_lora_initialization_checkpoint(self) -> None:
        """
        Initialize the current AV2AV LoRA from another pretrained LoRA.

        Only overlapping LoRA A/B tensors are copied.

        The following parameters keep their current initialization:
        1. AV2AV LoRA layers not present in the source Motion Track LoRA.
        2. SpatialTrackEncoder parameters in modules_to_save.

        This function does not restore:
        - optimizer
        - scheduler
        - RNG state
        - global step
        ...
        if lora_cfg is None or lora_cfg.init_checkpoint is None:
            logger.info("No LoRA initialization checkpoint configured.")
            return

        checkpoint_path = Path(lora_cfg.init_checkpoint).expanduser().resolve()

        if not checkpoint_path.is_file():
            raise FileNotFoundError(
                f"LoRA initialization checkpoint does not exist: "
                f"{checkpoint_path}"
            )

        logger.info(
            "Initializing AV2AV LoRA from pretrained checkpoint: "
            f"{checkpoint_path}"
        )

        # ---------------------------------------------------------
        # 1. Load source Motion Track LoRA
        # ---------------------------------------------------------
        source_file_state = load_file(
            str(checkpoint_path),
            device="cpu",
        )

        source_lora_state: dict[str, Tensor] = {}

        for source_key, source_tensor in source_file_state.items():
            canonical_key = self._normalize_peft_checkpoint_key(source_key)

            # 只复制 LoRA A/B，不复制其他完整模块
            if not self._is_lora_weight_key(canonical_key):
                continue

            if canonical_key in source_lora_state:
                raise RuntimeError(
                    f"Duplicate LoRA key in initialization checkpoint: "
                    f"{canonical_key}"
                )

            source_lora_state[canonical_key] = source_tensor

        if not source_lora_state:
            raise RuntimeError(
                "No LoRA A/B tensors were found in initialization checkpoint: "
                f"{checkpoint_path}"
            )

        # ---------------------------------------------------------
        # 2. Check that every source LoRA module has both A and B
        # ---------------------------------------------------------
        source_module_names = {
            key.rsplit(".lora_", maxsplit=1)[0]
            for key in source_lora_state
        }
        ...
        for module_name in sorted(source_module_names):
            ...
            if (
                key_a not in source_lora_state
                or key_b not in source_lora_state
            ):
                incomplete_modules.append(module_name)

        if incomplete_modules:
            preview = "\n".join(
                f"  - {name}" for name in incomplete_modules[:20]
            )
            raise RuntimeError(
                "The source LoRA checkpoint contains incomplete A/B pairs:\n"
                f"{preview}"
            )

        # ---------------------------------------------------------
        # 3. Obtain the complete current AV2AV PEFT state
        #
        # This includes:
        #   - all current AV2AV LoRA tensors
        #   - SpatialTrackEncoder from modules_to_save
        #
        # We retain all of them, and only overwrite matching Motion LoRA tensors.
        # ---------------------------------------------------------
        current_state, canonical_to_native = (
            self._get_current_peft_state_and_key_map()
        )
        ...
        missing_destination_keys: list[str] = []
        mismatched_shapes: list[
            tuple[str, tuple[int, ...], tuple[int, ...]]
        ] = []
        loaded_keys: list[str] = []

        for canonical_key, source_tensor in source_lora_state.items():
            native_key = canonical_to_native.get(canonical_key)

            if native_key is None:
                missing_destination_keys.append(canonical_key)
                continue
            ...
            merged_state[native_key] = source_tensor.to(
                device=destination_tensor.device,
                dtype=destination_tensor.dtype,
            )

            loaded_keys.append(canonical_key)

        # ---------------------------------------------------------
        # 4. Strict compatibility checks
        # ---------------------------------------------------------
        if mismatched_shapes:
            preview = "\n".join(
                (
                    f"  - {key}: source={source_shape}, "
                    f"destination={destination_shape}"
                )
                for key, source_shape, destination_shape
                in mismatched_shapes[:20]
            )

            raise RuntimeError(
                "Motion Track LoRA and AV2AV LoRA have incompatible tensor "
                "shapes. This is normally caused by different LoRA ranks:\n"
                f"{preview}\n\n"
                "Set the AV2AV LoRA rank equal to the source Motion Track "
                "LoRA rank."
            )
        ...
        if missing_destination_keys:
            preview = "\n".join(
                f"  - {key}"
                for key in missing_destination_keys[:20]
            )

            raise RuntimeError(
                "Some Motion Track LoRA tensors have no corresponding "
                "destination in the current AV2AV LoRA:\n"
                f"{preview}\n\n"
                "Check that:\n"
                "  1. Both checkpoints use the same LTX-2.3 base model.\n"
                "  2. AV2AV target_modules is a superset of the Motion "
                "Track LoRA target modules.\n"
                "  3. Transformer module names have not changed."
            )

        if not loaded_keys:
            raise RuntimeError(
                "The Motion Track checkpoint did not match any current "
                "AV2AV LoRA tensors."
            )

        # ---------------------------------------------------------
        # 5. Load the complete merged PEFT state
        #
        # Important:
        # Pass self._transformer, not get_base_model().
        # merged_state also contains the current SpatialTrackEncoder state,
        # preventing modules_to_save from being absent.
        # ---------------------------------------------------------
        set_peft_model_state_dict(
            self._transformer,
            merged_state,
        ...
        destination_lora_keys = {
            key
            for key in canonical_to_native
            if self._is_lora_weight_key(key)
        }

        newly_initialized_lora_keys = (
            destination_lora_keys - set(loaded_keys)
        )

        logger.info(
            "Successfully initialized AV2AV LoRA from Motion Track LoRA:\n"
            f"  source LoRA tensors: {len(source_lora_state)}\n"
            f"  loaded LoRA tensors: {len(loaded_keys)}\n"
            f"  loaded LoRA modules: {len(loaded_module_names)}\n"
            f"  newly initialized AV2AV-only LoRA tensors: "
            f"{len(newly_initialized_lora_keys)}\n"
            "  SpatialTrackEncoder: kept at its own initialization"
        )


    @staticmethod
    def _normalize_peft_checkpoint_key(key: str) -> str:
        """
        Convert PEFT/LTX/ComfyUI keys to one canonical key format.

        Examples:
            diffusion_model.transformer_blocks.0.xxx.lora_A.weight
                -> transformer_blocks.0.xxx.lora_A.weight

            base_model.model.transformer_blocks.0.xxx.lora_A.weight
                -> transformer_blocks.0.xxx.lora_A.weight
        """
...
def prepare_track_for_audio(
    track_xy: Tensor,
    track_valid: Tensor | None,
    *,
    audio_seq_len: int,
    device: torch.device,
) -> tuple[Tensor, Tensor]:
    """
    Convert video-frame tracks to audio-token-aligned tracks.

    Input:
        track_xy:    [B, T_track, 2]
        track_valid: [B, T_track] or None

    Output:
        track_xy:    [B, T_audio, 2], float32
        track_valid: [B, T_audio], bool
    """
    if track_xy.ndim != 3 or track_xy.shape[-1] != 2:
        raise ValueError(
            "track_xy must have shape [B, T, 2], "
            f"but got {tuple(track_xy.shape)}"
        )
    ...
    batch_size, track_len, _ = track_xy.shape
    ...
    if track_valid is None:
        ...
        if track_valid.shape != (batch_size, track_len):
            raise ValueError(
                "track_valid must have shape [B, T], "
                f"expected {(batch_size, track_len)}, "
    ...
    # 121 video-frame coordinates -> 122 audio tokens.
    if track_len != audio_seq_len:
