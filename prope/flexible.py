...
class ModalityConfig(BaseModel):
    ...
    track: str | None = Field(
        default=None,
        description=(
            "Optional directory under preprocessed_data_root/.precomputed "
            "containing spatial-track tensors. Intended for audio Track-PRoPE."
        ),
    )
...
class FlexibleStrategyConfig(TrainingStrategyConfigBase):
    ...
    @model_validator(mode="after")
    def validate_track_configuration(
        self,
    ) -> "FlexibleStrategyConfig":
        """
        Track-PRoPE currently attaches the spatial track to the audio
        modality, so the track directory may only be configured for audio.
        """
        if (
            self.video is not None
            and self.video.track is not None
        ):
            raise ValueError(
                "The 'track' data source must be configured under "
                "training_strategy.audio, not training_strategy.video."
            )
    ...
    def get_data_sources(self) -> dict[str, str]:
        ...
        Return:
            directory name under .precomputed
                -> key exposed by PrecomputedDataset
        """
        ...
        if self.audio is not None:
            ...
            # -----------------------------------------------------
            # Track-PRoPE data source
            # -----------------------------------------------------
            if self.audio.track is not None:
                track_dir = self.audio.track

                if track_dir in sources:
                    raise ValueError(
                        "The configured track directory conflicts with "
                        "another precomputed data directory:\n"
                        f"  track directory: {track_dir}\n"
                        f"  existing output key: {sources[track_dir]}"
                    )

                # .precomputed/<track_dir> -> batch["track"]
                sources[track_dir] = "track"

        for modality_config in (
            self.video,
            self.audio,
        ):
            if modality_config is None:
...
class FlexibleStrategy(TrainingStrategy):
    ...
    def prepare_customize_training_inputs(
        self,
        batch: dict[str, Any],
        timestep_sampler: TimestepSampler,
        _transformer
        ...
        """Prepare training inputs by processing video and audio modalities."""
        video_result = self._process_modality(self.config.video, batch, "video", timestep_sampler)
        audio_result = self._process_tracker_encoder_modality(self.config.audio, batch, "audio", timestep_sampler, _transformer)
    ...
    def _process_tracker_encoder_modality(
        self,
        modality_config: ModalityConfig | None,
        batch: dict[str, Any],
        modality_key: str,
        ...
        for cond in modality_config.conditions:
            if isinstance(cond, ReferenceConditionConfig):
                noisy_latents, positions, timesteps, loss_mask, targets = self._apply_track_encoder_reference_condition(
                    noisy_latents=noisy_latents,
                    positions=positions,
                    timesteps=timesteps,
                    loss_mask=loss_mask,
    ...
    def _apply_track_encoder_reference_condition(
        self,
        noisy_latents: Tensor,
        positions: Tensor,
        timesteps: Tensor,
        ...
        # Load and patchify condition latents
        cond = self._patchify_latent_data(batch[config.latents_dir], modality_key)
        video_cond = self._patchify_latent_data(batch["reference_latents"], "video")
        
        video_cond_latents = video_cond.latents

        # process 
        spatial_tracker_cond_latent = transformer.spatial_track_encoder(video_cond_latents)
        cond.latents = spatial_tracker_cond_latent
        cond_latents = cond.latents
        ...
        # Generate condition positions
        # video_cond_positions = self._get_video_positions(
        #     num_frames=video_cond.num_frames,
        #     height=video_cond.height,
        #     width=video_cond.width,
        #     batch_size=batch_size,
        #     fps=video_cond.fps,
        #     device=device,
        # )
        ...
        #     temporal_sf = self.reference_temporal_scale_factor or 1
        #     if spatial_sf != 1 or temporal_sf != 1:
        #         video_cond_positions = video_cond_positions.clone()
        #         if temporal_sf != 1:
        #             # Ref positions are already at the ref's effective fps (source_fps / S,
        #             # stored by process_videos.py). Shift by (S - 1) / target_fps so ref's
        #             # last patch aligns with target's last; clamp the causal patch at 0.
        #             t_target = positions[:, 0, 0:1, 1:2]  # = 1 / target_fps
        #             video_cond_positions[:, 0, ...] = torch.clamp(
        #                 video_cond_positions[:, 0, ...] - (temporal_sf - 1) * t_target, min=0
        #             )
        #         if spatial_sf != 1:
        #             video_cond_positions[:, 1, ...] *= spatial_sf
        #             video_cond_positions[:, 2, ...] *= spatial_sf
