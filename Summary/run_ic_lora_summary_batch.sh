...
CHECKPOINT_DIR="/data/vjuicefs_ai_camera_jgroup_acadmic/public_data/11194554/WorkSpace/LTX-2/packages/ltx-trainer/outputs/av2av_ic_lora_full_train_monodiff_summary_v1/checkpoints"
LORA_PATH="${CHECKPOINT_DIR}/lora_weights_step_05250.safetensors"
TRACK_SUMMARY_WEIGHTS="${CHECKPOINT_DIR}/track_summary_step_05250.safetensors"

# Must match summary_source_mode used during training:
# track_xy | reference_tokens | reference_guided_target | hybrid
SUMMARY_SOURCE_MODE="reference_guided_target"
OUTPUT_PATH="evaluation_results/"
CUDA_DEVICE="3"

# Optional modules can be enabled by adding either of these arguments below:
...
CUDA_VISIBLE_DEVICES="${CUDA_DEVICE}" python -m ltx_pipelines.ic_lora_customize_batch \
  ...
  --track-summary-weights "${TRACK_SUMMARY_WEIGHTS}" \
  --summary-source-mode "${SUMMARY_SOURCE_MODE}" \
