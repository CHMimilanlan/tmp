...
class ValidationRunner:
    ...
    def _generate_sample(
        ...
        if generate_audio or audio_frozen:
            if audio_frozen:
                ...
                if transformer.spatial_track_encoder is None:
                    audio_state = self._apply_audio_conditionings(audio_state, audio_tools, sample, cached_media, device)
                else:
                    audio_state = self._apply_customize_audio_conditionings(audio_state, audio_tools, video_tools, sample, cached_media, device, transformer)
    ...
    @staticmethod
    def _apply_customize_audio_conditionings(
        state: LatentState,
        audio_tools: AudioLatentTools,
        ...
        video_media = cached_media.conditions.get(0)
        video_latent = video_media.latent.to(device=device, dtype=torch.bfloat16)
        video_latent = video_tools.patchifier.patchify(video_latent)
        breakpoint()
        latent = transformer.spatial_track_encoder(video_latent)
        
        tokens = latent
        
        # breakpoint()
        # ref_shape = AudioLatentShape.from_torch_shape(latent.shape)
        ref_shape = AudioLatentShape.from_torch_shape(torch.Size([1, 8, 122, 16]))

        positions = audio_tools.patchifier.get_patch_grid_bounds(
            output_shape=ref_shape,
            device=device,
        )
