...
def preprocess_dataset(  # noqa: PLR0912, PLR0913, PLR0915
    ...
    # Validate: need at least caption (unless caption processing is skipped)
    if "caption" not in roles and not skip_caption:
        raise ValueError(
    ...
    if skip_caption or "caption" not in roles:
        logger.info("Skipping caption encoding (Phase 1)")
    else:
...
def main(  # noqa: PLR0913
    ...
    text_encoder_path: str = typer.Option(
        default="",
        help="Path to Gemma text encoder directory (not required when --skip-caption is set)",
    ),
    ...
    skip_caption: bool = typer.Option(
        default=False,
        help="Skip caption text-encoding phase entirely (useful when only audio/video latents are needed, "
        "or when 'conditions/' has already been precomputed).",
    ),
) -> None:
    """Preprocess a media dataset for LTX-2 training.
